...
// @ts-expect-error
import { forceSpeaker } from "../mobile/audio-mode/forceSpeaker";
...
import { CALENDAR_NOTIFICATION_ID, NOTIFICATION_ICON, NOTIFICATION_TIMEOUT_TYPE } from "../notifications/constants";
...
MiddlewareRegistry.register((store) => (next) => (action) => {
    ...
    switch (action.type) {
        ...
        case "LOCAL_AUDIO_ROUTE_CHANGED": {
            ...
            console.log("📤 Sending AUDIO_ROUTE_CHANGED:", action.route);

            conference.sendEndpointMessage("", {
                name: "AUDIO_ROUTE_CHANGED",
                route: action.route, // 'PHONE' | 'SPEAKER'
            });
        ...
        case ENDPOINT_MESSAGE_RECEIVED: {
            ...
            if (
                data.name === LOWER_HAND_MESSAGE &&
                participant.isModerator() &&
                !getDisableLowerHandByModerator(getState())
            ) {
                dispatch(raiseHand(false));
            }
            if (data?.name === "FORCE_SPEAKER" && participant?.isModerator?.()) {
                console.log("🔊 FORCE_SPEAKER received from moderator");
                forceSpeaker();
            }
            if (data?.name === "AUDIO_ROUTE_CHANGED") {
                console.log("📥 AUDIO_ROUTE_CHANGED from", participant.getId(), "route:", data.route);

                dispatch({
                    type: "REMOTE_AUDIO_ROUTE_CHANGED",
                    participantId: participant.getId(),
                    route: data.route, // PHONE | SPEAKER
                });
            }
    ...
    if (isEmbedded() && state["features/base/config"].disableIframeAPI && !isVpaasMeeting(state) && !allowIframe) {
        ...
        if (browser.isReactNative()) {
            ...
            dispatch(
                showWarningNotification(
                    {
                        description: translateToHTML(i18next.t.bind(i18next), translationKey, {
                            domain,
                            jaasDomain,
                            timeout: IFRAME_DISABLED_TIMEOUT_MINUTES,
                        }),
...
function _maybeDisplayCalendarNotification({ dispatch, getState }: IStore) {
    ...
    const currentConferenceURL = locationURL ? getURLWithoutParamsNormalized(locationURL) : "";
...
function _calendarNotification({ dispatch, getState }: IStore, eventToShow: any) {
    ...
    const description = getLocalizedDateFormatter(eventToShow.startDate).fromNow();
    const icon = NOTIFICATION_ICON.WARNING;
    const title =
        eventToShow.startDate < now && eventToShow.endDate > now
