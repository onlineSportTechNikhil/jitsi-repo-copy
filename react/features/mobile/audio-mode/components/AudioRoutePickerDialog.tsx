...
class AudioRoutePickerDialog extends Component<IProps, IState> {
    ...
    static getDerivedStateFromProps(props: IProps) {
        const { _devices: devices, t, dispatch } = props;
        ...
        const audioDevices = [];
        let audioRoute: "PHONE" | "SPEAKER" | null = null;
        ...
        for (const device of devices) {
            ...
            if (device.type === "EARPIECE" && device.selected) {
                audioRoute = "PHONE";
            }

            if (device.type === "SPEAKER" && device.selected) {
                audioRoute = "SPEAKER";
            }
        ...
        // ✅ ONLY REDUX ACTION — no native / no moderator here
        ...
        return () => {
            if (_localParticipant?.id) {
                dispatch({
                    ...
                    participantId: _localParticipant.id, // ✅ YAHI MISSING THA
                    route: device.type,
                });
            }
        ...
        return (
            ...
            >
                <View style={styles.deviceRow as ViewStyle}>
                    <Icon
                        src={icon}
                        style={[styles.deviceIcon, bottomSheetStyles.buttons.iconStyle, selectedStyle] as StyleType[]}
                    />
                    <Text
                        style={[styles.deviceText, bottomSheetStyles.buttons.labelStyle, selectedStyle] as TextStyle[]}
                    >
                        {text}
                    </Text>
                </View>
