...
interface IProps {
    ...
    /**
     * Participant ka current audio device (EARPIECE / SPEAKER / HEADPHONES)
     */
    _participantDevice?: string;
}
...
class Thumbnail extends PureComponent<IProps> {
    ...
    _onClick() {
        ...
        // 🔥 MODERATOR → FORCE SPEAKER (only dispatch)
        if (_isModerator) {
            console.log("🔥 Moderator clicked user:", _participantId);

            dispatch({
                type: "FORCE_REMOTE_SPEAKER",
                participantId: _participantId,
            });

            return; // ❌ pin / toolbox ko block kar do
        }
 ...
 * @returns {Object}
 */
// Thumbnail _mapStateToProps (working version)
function _mapStateToProps(state: IReduxState, ownProps: any) {
    const { participantID, tileView } = ownProps;
    console.log("[_mapStateToProps] ownProps:", ownProps);

    const participant = getParticipantByIdOrUndefined(state, participantID);
    console.log("[_mapStateToProps] participant object:", participant);

    if (!participant) {
        // Agar participant exist nahi karta, to fallback return
        console.log("[_mapStateToProps] Participant not found, returning undefined for _participantDevice");
        return {
            _participantDevice: undefined,
        };
    }

    const id = participant.id;
    console.log("[_mapStateToProps] participant id:", id);
    const localParticipantId = getLocalParticipant(state)?.id;
    console.log("[_mapStateToProps] local participant id:", localParticipantId);

    // Redux se participant ka current audio device
    const participantDevice =
        state["features/mobile/audio-mode"].participantsDevices[participant.id] ??
        state["features/mobile/audio-mode"].participantsDevices[localParticipantId] ??
        "DEFAULT";
    console.log("[_mapStateToProps] participantDevice from redux:", participantDevice);
    ...
    console.log("[_mapStateToProps] Computed props:", {
        ...
        _localVideoOwner: state["features/shared-video"].ownerId === getLocalParticipant(state)?.id,
        _participantId: id,
        ...
        _participantDevice: participantDevice,
    });
